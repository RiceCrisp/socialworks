<?php
// Fill meta box
function _sw_sitemap_meta_fields() {
...
// Create meta box
function _sw_sitemap_meta() {
  add_meta_box('sitemap-meta-box', 'Sitemap', '_sw_sitemap_meta_fields', get_post_types(), 'normal', 'high');
}
add_action('add_meta_boxes', '_sw_sitemap_meta');

// Save meta values
function _sw_save_sitemap_meta($post_id) {
...
add_action('save_post', '_sw_save_sitemap_meta');

function _sw_sitemap_menu() {
  add_options_page('Sitemap', 'Sitemap', 'manage_options', 'sitemap', '_sw_sitemap_page');
}
add_action('admin_menu', '_sw_sitemap_menu');

function _sw_sitemap_fields() {
  register_setting('sitemap', 'last_mod');
  register_setting('sitemap', 'sitemap_post_types');
}
add_action('admin_init', '_sw_sitemap_fields');

// Sitemap page
function _sw_sitemap_page() { ?>
...
function _sw_activate_sitemap_routine() {
  if (!wp_next_scheduled('_sw_sitemap_routine')) {
    wp_schedule_event(time(), 'daily', '_sw_sitemap_routine');
  }
}
add_action('after_setup_theme', '_sw_activate_sitemap_routine');

add_action('_sw_sitemap_routine', '_sw_sitemap');

function _sw_deactivate_sitemap_routine() {
  wp_clear_scheduled_hook('_sw_sitemap_routine');
}
add_action('switch_theme', '_sw_deactivate_sitemap_routine');
