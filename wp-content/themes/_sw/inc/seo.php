      ...
      <li class="row">
        <div class="col-xs-12">
          ...
          <ul id="seo-advanced-options">
            <?php
            $amp = array_key_exists(get_post_type(), get_option('amp') ?: array()); ?>
            <li <?= $amp ? 'class="disabled"' : ''; ?>>
              <label for="seo-canonical">Canonical URL <small><?= $amp ? 'Disabled for AMP' : 'If left blank, defaults to page url'; ?></small></label>
              <input id="seo-canonical" name="seo-canonical" type="text" value="<?= $seo_canonical; ?>" <?= $amp ? 'disabled' : ''; ?> />
            </li>
...
// Create meta box
function _sw_seo_meta() {
  add_meta_box('seo-meta-box', 'SEO', '_sw_seo_meta_fields', get_post_types(), 'normal', 'high');
}
add_action('add_meta_boxes', '_sw_seo_meta');

// Save meta values
function _sw_save_seo_meta($post_id) {
...
add_action('save_post', '_sw_save_seo_meta');

// Exclude pages from internal search
function _sw_search_filter($query) {
	if ($query->is_search) {
...
add_action('pre_get_posts', '_sw_search_filter');
