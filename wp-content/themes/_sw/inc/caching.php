...
function _sw_caching_menu() {
	add_options_page('Caching', 'Caching', 'manage_options', 'caching', '_sw_caching_page');
}
add_action('admin_menu', '_sw_caching_menu');

// Register caching fields
function _sw_caching_fields() {
  register_setting('caching', 'cache');
}
add_action('admin_init', '_sw_caching_fields');

// Create caching page
function _sw_caching_page() { ?>
...
function _sw_update_caching($old, $new) {
...
add_action('update_option_cache', '_sw_update_caching', 10, 2);
