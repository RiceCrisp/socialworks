...
function _sw_post_selector_types() {
...
add_action('wp_ajax__sw_post_selector_types', '_sw_post_selector_types');
add_action('wp_ajax_nopriv__sw_post_selector_types', '_sw_post_selector_types');

// Post selector posts
function _sw_post_selector_posts() {
...
add_action('wp_ajax__sw_post_selector_posts', '_sw_post_selector_posts');
add_action('wp_ajax_nopriv__sw_post_selector_posts', '_sw_post_selector_posts');

// SVG selector
function _sw_svg_selector() {
  $svgs = get_option('svg');
  $output = '<div class="svg-selector-container"><div class="svg-selection"><svg><use xlink:href="/wp-content/themes/_sw/template-parts/sprites.svg#' . ($_REQUEST['value'] ?: '') . '"></use></svg><button><span class="dashicons dashicons-arrow-down"></span></button></div><ul><li><input id="' . $_REQUEST['id'] . '-id-empty" name="' . $_REQUEST['id'] . '-id" type="radio" value="" ' . (!$_REQUEST['value'] ? 'checked' : '') . ' /><label for="' . $_REQUEST['id'] . '-id-empty">No Icon</label></li>';
  if ($svgs) {
    foreach ($svgs as $svg) {
...
add_action('wp_ajax__sw_svg_selector', '_sw_svg_selector');
add_action('wp_ajax_nopriv__sw_svg_selector', '_sw_svg_selector');

// Infinite scroll posts
function _sw_get_more_posts() {
  ob_start();
...
add_action('wp_ajax__sw_get_more_posts', '_sw_get_more_posts');
add_action('wp_ajax_nopriv__sw_get_more_posts', '_sw_get_more_posts');

// Send Email
function _sw_send_email() {
  $res = false;
...
add_action('wp_ajax__sw_send_email', '_sw_send_email');
add_action('wp_ajax_nopriv__sw_send_email', '_sw_send_email');

// Generate Sitemap
function _sw_generate_sitemap() {
  if (_sw_sitemap()) {
...
add_action('wp_ajax__sw_generate_sitemap', '_sw_generate_sitemap');
add_action('wp_ajax_nopriv__sw_generate_sitemap', '_sw_generate_sitemap');

// Return csv of pages for bulk edit
function _sw_get_bulk() {
...
add_action('wp_ajax__sw_get_bulk', '_sw_get_bulk');
add_action('wp_ajax_nopriv__sw_get_bulk', '_sw_get_bulk');

// Update meta data from uploaded bulk edit csv
function _sw_set_bulk() {
...
add_action('wp_ajax__sw_set_bulk', '_sw_set_bulk');
add_action('wp_ajax_nopriv__sw_set_bulk', '_sw_set_bulk');
