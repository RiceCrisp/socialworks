...
function _ws_menu_walker_fields($item_id, $item) {
  if ($item->type === 'custom') :
    if ($item->url === '#custom_heading' || $item->url === '#custom_search' || $item->url === '#custom_column') : ?>
    ...
    <div id="icon-menu-<?= $item_id; ?>" class="svg-select" aria-labelledby="icon-button-<?= $item_id; ?>">
      <button data-value="">
...
function _ws_add_menu_meta_box() {
  ...
  add_meta_box('custom-menu-item-column', __('Column', '_ws'), '_ws_menu_column_meta_box', 'nav-menus', 'side');
}
...
function _ws_menu_item_labels($menu_item) {
  ...
  if ($menu_item->url === '#custom_column') {
    $menu_item->type_label = __('Column', '_ws');
  }
  return $menu_item;
}
