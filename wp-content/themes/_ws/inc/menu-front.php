...
function _ws_custom_desktop_menu_output($menuItem, $depth) {
  $output = '';
  $submenuClasses = $menuItem->url === '#custom_column' ? ['menu-column'] : ['sub-menu'];
  if ($menuItem->children) {
    $childColumns = array_filter($menuItem->children, function($child) {
      return $child->url === '#custom_column';
    });
    if (count($childColumns) > 0) {
      array_push($submenuClasses, 'has-columns', 'has-' . count($childColumns) . '-columns');
    }
  }
  if ($menuItem->url !== '#custom_column') {
    $output .= _ws_start_el($menuItem, $depth, ['item_spacing' => 'discard']);
    $depth++;
...
function _ws_custom_mobile_menu_output($menuItem, $depth) {
  ...
  if ($menuItem->children) {
    $depth++;
    $submenuClasses[] = 'depth-' . ($depth + 1);
    $output .= do_shortcode('<button class="dropdown" aria-haspopup="true" aria-expanded="false" aria-label="' . __('Toggle Sub Menu', '_ws') . '">[svg id="caret-down"]</button>');
    $output .= '<ul class="' . implode(' ', $submenuClasses) . '">';
      foreach ($menuItem->children as $child) {
        $output .= _ws_custom_mobile_menu_output($child, $depth);
      }
...
function _ws_start_el($item, $depth = 0, $args = [], $id = 0) {
  ...
  $item_output .= $image ? '<img class="menu-image header-lazy-load" src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" data-src="' . wp_get_attachment_image_src($image, 'small')[0] . '" alt="Menu Image" />' : '';
...
function _ws_walker_nav_menu_start_el($item_output, $item, $depth, $args) {
  ...
  if ($item->url === '#custom_heading') {
    $item_output = '<p class="menu-heading">' . $item->post_title . '</p>';
  }
...
function _ws_submenu_depth_class($classes, $args, $depth) {
  array_push($classes, 'depth-' . ($depth + 1));
  return $classes;
}
